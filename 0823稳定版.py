...
def bigquant_run(context, data):
    ...
    if len(positions) > 0:
        ...
        for instrument in positions.keys():
            ...
            # 亏5%并且为可交易状态就止损
            if rate <= -0.057 and return0 < 1.03 and data.can_trade(
                    context.symbol(instrument)) and instrument not in stock_sold:
            ...
            if instrument not in stock_sold and context.instrument_hold_days[
                instrument] >= 7 and stock_market_price / stock_cost - 1 < 0.1 and close != high and data.can_trade(
                    context.symbol(instrument)):
                context.order_target_percent(context.symbol(instrument), 0)
                cash_for_sell -= positions[instrument]
                cash_for_buy += positions[instrument]
                print(today, instrument, '持股满7天卖出', 'stock_market_price', stock_market_price, 'stock_cost',
                      stock_cost, '天数', context.instrument_hold_days[instrument])
                current_stoploss_stock.append(instrument)
                context.instrument_high_price.pop(instrument)
                context.instrument_hold_days.pop(instrument)
                stock_sold.append(instrument)
