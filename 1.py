...
def bigquant_run(context, data):
    ...
    # print('is_staging',is_staging,'cash_avg',cash_avg,'cash_for_buy',cash_for_buy,'cash_for_sell',cash_for_sell)
    ...
    if len(positions) > 0:
        ...
        for instrument in positions.keys():
            ...
            # 持股满7天且当日非涨停则卖出
            # if context.instrument_hold_days[instrument] >= 7 and stock_market_price / stock_cost - 1 < 0.2 and close != high and data.can_trade(context.symbol(instrument)) and instrument not in stock_sold:
            ...
            if rate <= -0.12 and data.can_trade(context.symbol(instrument)) and instrument not in stock_sold:
                context.order_target_percent(context.symbol(instrument), 0)
