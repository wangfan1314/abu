...
for k in range(len(m24.result[0])):
    try:
        ...
        df_empty = pd.DataFrame(res_tmp, columns=['时间', '总收益', '最大回撤', 'alpha', '夏普比率', '过滤条件'])
        try:
            Result = pd.read_csv('过滤条件批量测试.csv', index_col=0)
            df_empty.to_csv('过滤条件批量测试.csv', header=False, mode='a', index=False)
        except Exception as e:
            df_empty.to_csv('过滤条件批量测试.csv', header=['时间', '总收益', '最大回撤', 'alpha', '夏普比率', '过滤条件'], mode='a', index=False)
        print('写入完成第{}组因子'.format(k))
    except:
        print('第{}组因子出错!请检查'.format(k))
        continue

df = pd.read_csv('过滤条件批量测试.csv')
df.sort_values('夏普比率', inplace=True, ascending=False)
df.to_csv('过滤条件批量测试.csv', header=['时间', '总收益', '最大回撤', 'alpha', '夏普比率', '过滤条件'], mode='w', index=False)
print('csv追加写入结束')
